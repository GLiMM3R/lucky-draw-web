<template>
  <div :id="`wheel-container-${randomIdRoulette}`" class="wheel-container" :class="[
    ...
    <!-- BASE WHEEL -->
    <div v-if="baseDisplay" class="wheel-base-container" :class="[{ 'wheel-base-container-shadow': baseDisplayShadow }]"
      :style="{
        width: `${baseSize}px`,
        height: `${baseSize}px`,
    ...
    <div class="wheel" :class="[`easing-${easing}`, { 'wheel-border': displayBorder }]" :style="{
      ...
      <div v-for="(item, index) in items" :key="item.id" class="wheel-item" :style="{
        transform: `rotate(${itemAngle * Number(index)}deg) skewY(${-(
          90 - itemAngle
        )}deg)`,
...
export default defineComponent({
  ...
  methods: {
    ...
    launchWheel() {
      ...
      const wheelElt = document.querySelector<HTMLDivElement>(`#wheel-container-${this.randomIdRoulette} .wheel`);
