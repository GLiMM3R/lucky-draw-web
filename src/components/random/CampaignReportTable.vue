<template>
    ...
    <VCard class="shadow">
        <v-data-table :groupBy="groupBy" :sort-by="sortBy" :headers="headers" :items="randomPrizesWithWinner"
            class="elevation-1">
...
const { randomPrizesWithWinner } = storeToRefs(randomPrizeStore);

await randomPrizeStore.fetchWinnerRandomPrizeBySlug(slug)

const sortBy = [{ key: 'createdAt', order: 'asc' }];
const groupBy = [{ key: 'prize.title', order: 'asc' }];
const headers = [
    {
        title: i18n.t('table.header.winner.customerId'),
        key: 'customerId',
    },
    {
